...
Define $s\in S_{<n}$ if and only if $\phi(t)=(s,a)$ for some $t< n$ and $a$. For every pair of states $s,s' \in S_{<n}$ let $R_{ss'}^{(1)}$ be the set of all $A_t$ such that $S_tA_t=S_{t+1}$ where $S_t=s$ and $S_{t+1}=s'$. Let $R_{ss'}^{(n)}$ be defined recursively as 
\[
R_{ss''}^{(n+1)} = \bigcup\limits_{\substack{s'\in S_{<n} \\ s'\ne s \\ s'\ne s''}} R_{ss'}^{(n)}R_{s's''}^{(n)}
\]
(Note that this is matrix multiplication with product and sum of formal languages as multiplicative and additive operations.) Taking the limit $R_{ss'}^{(\infty)}$ yields transitive closure. Computing $R_{ss'}^{(\infty)}$ can be done in finite number of steps because $S_{<n}$ is finite. If $r\in R_{ss}^{(\infty)}$ then $sr=s$ and $r=e$. The final result $\mathcal{R}\subset A^*$ is
\[
