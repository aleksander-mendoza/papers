...
Instead of implementing winner-takes-all mechanism with $\argmax$ function, I propose its soft version. It is a generalization of ECC networks that models inhibition in a more biologically detailed manner. 
...
\begin{lstlisting}
1. initialize $y_k=\emptyset$ for all $k$
...
1. for every $j$
   2. set $y_j=0$ if  $\exists_k u_{kj}=1 \wedge s_j<s_k$
        otherwise set $y_j=1$
\end{lstlisting}
because  $y_{k}$ could be $0$ only if there exists some other $y_l=1$ that shunned it. Furthermore $U$ is reflexive if $u_{jj}=1$ for all $j$, but reflexivity does not change the outcome because $s_j > s_j$ is always false. $U$ is  symmetric when  $u_{jk}=u_{kj}$. If $U$ is transitive, symmetric and (optionally) reflexive then $U$ becomes an equivalence relation on $y$. Then the algorithm simplifies to
\begin{lstlisting}
1. for every equivalence class $[j]$
    2. find $k = \argmax s_i$ over all $i \in [j]$
    3. set $y_k=1$ 
    4. set $y_i=0$ for all $i\in [j]$ where $i\ne k$
\end{lstlisting}
It can be seen that multilayer convolutional ECC networks are a special case of soft-WTA.
When all entries of $U$ matrix are $1$, then there can be only one winner $y_k=1$ for $k=\argmax \boldsymbol{s}$, which corresponds to an ECC network with a single inhibitory neuron. 
This leads to the conclusion that, in the general case, $U$ does not require clear division into layers (much like real brain).
