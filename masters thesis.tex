...
\]
where $C=\mathbb{E}(\boldmath{x}^T  \boldmath{x})$ is the ``correlation'' matrix. In the case of sparse binary vectors it would be undesirable to say that $x_h,x_i$ are correlated if they are always silent together $x_i=x_h=0$. Therefore  $C$ is not a correlation according to ``classical'' definition (but would be if we first whitened $\boldsymbol{x}$). 
...
\]
\[
\mathbb{E}(|s_j - s_k|) = \mathbb{E}\big(\sqrt{(s_j - s_k)^2}\big) = 
