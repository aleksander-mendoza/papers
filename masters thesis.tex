...
s = a_1 a_2 a_3 ... a_t
\]
of one or more elements $a_1,a_2,a_3,...a_t$ of $A$. In order to preserve the same notation when $S$ is a monoid or semigroup, let us assume that if $S$ is a group then for every $a\in A$ its inverse $a^{-1}$ is also in $A$ (we won't be using $A^{\pm}$ notation like in some books). This consequently requires that every presentation $\langle A | \mathcal{R} \rangle$ includes relations $a=a^{-1}$ in $\mathcal{R}$ (or that they can be derived from other relations).
...
For an agent moving in a large room, the state space $S$ might look like a group of two-dimensional translations... until it hits a wall. To rigorously discuss this phenomenon, we need to assume that $S$ is a monoid generated by $A$. Take a subset $E \subset S$ but not necessarily a submonoid. We define that $E$ is isomorphic to a group $G$ if there exists a mapping $f:S \rightarrow G$ such that $f(e)a=f(ea)$ for all $e\in E$, $a\in A$. It is a relaxed definition of group isomorphism, where $E$ need not be closed under the group operation. If two sets $E_1,E_2 \subset S$ are isomorphic to $G$ then so is their union and intersection. A point $s\in S$ Let us refer to $E$ as a \textit{group manifold} embedded in $S$ which locally resembles $G$.

Let us first consider discrete topology induced by metric $d(s,s')=t$ where $s'=sa_1...a_{t}$ is the shortest trajectory from $s$ to $s'$. 
