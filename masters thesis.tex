...
Instead of implementing winner-takes-all mechanism with $\argmax$ function, let us consider its ``softer'' version. 
Suppose $\boldsymbol{x} \in \{0,1\}^n$ and $\boldsymbol{y} \in \{0,1\}^m$ are two consecutive fully connected layers with weight matrix $W\in \mathbb{R}^{n\times m}$ between them. The input $\boldsymbol{x}$ causes excitation $s=\boldsymbol{x} W$.
Let us now model inhibition 


 Neuron $y_j=1$ fires if the voltage $s_j$ exceeds certain threshold $s_j > d$. Otherwise its value remains $y_j=0$. There is no limit on the number of ``winners''. It could in theory so happen that all neurons $\boldsymbol{y}$ fire at the same time. Input patterns $\boldsymbol{x}$ are drawn from probability distribution $p(\boldsymbol{x})$. We can define probability of output $p(\boldsymbol{y})$ where
\[p(y_j)=p(s_j > d^{+})=p(\boldsymbol{x} W_j > d)\]
Note that $p(\boldsymbol{y}|\boldsymbol{x})$ is not interesting, because $\boldsymbol{x}$ exactly determines $\boldsymbol{y}$. The bits $y_j$ should not be treated as independent random variables, as they deterministically depend on $\boldsymbol{x}$ and therefore have non-trivial correlations $p(y_j|y_k)$. In order to implement ``soft WTA'' we can use inhibitory mechanisms to decorrelate the bits $p(y_j|y_k)\approx 0$. In the case of ECC networks, the bits were mutually exclusive, leading to $p(y_j|y_k) = 0$. Now we want to introduce negative weights $V \in \mathbb{R}^{m\times m}$ of effective inhibitory neurons (effective means that we do not introduce additional inhibitory neurons, but rather simulate their fast-spiking dynamics by allowing excitatory neurons $\boldsymbol{y}$ to have inhibitory synapses $V$ as well) and use Hebbian learning to find which pairs $p(y_j|y_k)$ occur frequently together and then increment the inhibitory connection from $y_k$ to $y_j$. Similarly as before, we could impose restriction that columns $W_j$ and $V_j$ are normalized to some constant magnitudes $\lVert W_j \rVert = a$ and $\lVert V_j \rVert = b$. This way each neuron $y_j$ needs to specialize by only listening to a subset of most important inputs $\boldsymbol{x}$ and inhibiting a subset of most correlated neighbors $\boldsymbol{y}$. 

This approach has two key advantages. First, we no longer need to manually specify which neurons are mutually exclusive. Hebbian learning will automatically find the right cliques of highly correlated neurons. Such approach is ``softer'' and will work even if some cliques overlap. Second, if an unexpected input appears that triggers multiple uncorrelated neurons to fire, they will not interfere with each other, because their inhibitory connections would be weak. This could preserve more information about the input and enhance out-of-distribution learning.
...
\subsection{Continuous and discrete computation}
So far this work has been focused on attempts to discretize the neurons and distill from them an elegant algorithm. We should better understand the justification for such approach as well as its limitations.
...
Even if the sole purpose of axonal delays was to maintain order, we should still consider whether continuous-time algorithms have more computational power. Is there something significant that could not be captured in discrete-time? 




ECC networks assume that discrete clock cycles could substitute continuous delays. 
