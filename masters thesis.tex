...
Instead of implementing winner-takes-all mechanism with $\argmax$ function, I propose its soft version. It is a generalisation of ECC networks that models inhibition in a more biologically detailed manner. 
Let $\boldsymbol{x} \in \{0,1\}^n$ and $\boldsymbol{y} \in \{0,1\}^m$ be input and output neurons with fully connected weight matrix $W\in \mathbb{R}^{n\times m}$ between them. The input $\boldsymbol{x}$ causes excitation $s=\boldsymbol{x} W$. Let $U\in \{0,1\}^{m\times m}$ be the matrix representing effective inhibitory synapses between $\boldsymbol{y}$ neurons, where $u_{kj}=1$ means that $y_k$ inhibits $y_j$.  The value of $s$ could be interpreted in terms of latency coding. Higher $s_j$ corresponds to higher (momentary) frequency and lower latency. We define that a neuron $y_j$ is shunned by $y_k$ if  $u_{kj}=1$, $y_k=1$ and $s_k>s_j$. We define that the neuron fires $y_j=1$ if and only if there does not exist any $y_k$ such that  $y_j$ is shunned by $y_k$. The algorithm for computing full vector $\boldsymbol{y}$ is as follows
\begin{lstlisting}
1. Initialise $y_k=\emptyset$ for all $k$. 
2. Find $k$ of the highest value $s_k$ such that $y_k=\emptyset$. 
     Terminate if no such $y_k$ exists. 
3. Set $y_j=0$ for all $j$ such that $u_{kj}=1$ and $s_j<s_k$.
4. Set $y_k=1$.
5. Go to step 2.
\end{lstlisting}
It is not possible to compute bits of $\boldsymbol{y}$ in parallel (unless we use spiking networks and truly simulate all latencies in continuous time). 

Several special cases are worth pointing out. Consider transitive $U$, meaning that if $u_{kj}=1$ and $u_{lk}=1$ then $u_{lj}=1$. Then it becomes possible to evaluate all bits independently and in parallel (on a digital computer)
\begin{lstlisting}
1. for every $j$
   2. set $y_j=0$ if  $\exists_k u_{kj}=1 \wedge s_j<s_k$
        otherwise set $y_j=1$
\end{lstlisting}
because  $y_{k}$ could be $0$ only if there exists some other $y_l=1$ that shunned it.
When all entries of $U$ matrix are $1$, then there can be only one winner $y_k=1$ for $k=\argmax \boldsymbol{s}$, thus  hard-WTA is a special case of soft-WTA. If $U$ is a block-diagonal matrix then soft-WTA becomes equivalent to hard-WTA with multiple mutually exclusive subpopulations of $\boldsymbol{y}$, thus multilayer convolutional ECC networks are a special case of soft-WTA, whereas more general $U$ does not require clear division into layers (much like real brain).

The product $s_j s_k$ is a proxy for determining
\[
...
Bits $y_j,y_k$ with similar $W_j,W_k$ will likely fire together and their ``correlation'' $\mathbb{E}(y_j y_k)$ will be high. Decorrelation, which is necessary for slow feature analysis, can be achieved by setting $u_{kj}=u_{jk}=1$. Of course if all $u_{kj}$ equal $1$ then all bits are mutually exclusive and decorrelation $p(y_j|y_k) = 0$ for $j\ne k$ is guaranteed but such arrangement is not very interesting. Therefore we should state that learning $U$ is the problem of minimising ``correlation'' $\mathbb{E}(\boldsymbol{y}^T \boldsymbol{y})$, while at the same time maximising sparsity of $U$. These two goals contradict each other, thus a trade-off  must be made. A reasonable assumption is that if an unusual input $\boldsymbol{x}$ causes two uncorrelated neurons $y_j,y_k$ (with distant $W_j,W_k$) to fire, they should not interfere with each other ($u_{kj}=u{kj}=0$). 


Let us generalise further by considering $U \in \mathbb{R}^{m\times m}$.
Neuron $y_j$ is shunned by $y_k$ if  $s_k-s_j>1-u_{kj}$ and $y_k=1$. Using continuous values allows us to incrementally adjust $U$ with Hebbian weight updates.  In biological neurons, inhibitory long-term potentiation (iLTP) \cite{inhibitory_plasticity} states that synapse ($u_{kj}$) is strengthened when presynaptic interneuron fires ($y_k=1$) and postsynaptic neuron is depolarised (high $s_j$) but does not fire ($y_j=0$). We shall give this rule a rigorous interpretation.

The probability $p(y_j\text{ shunned by} y_k)$ is by definition equal to
\[
p(s_k-s_j>1-u_{kj} \wedge y_k=1) = p(y_k)p(s_k-s_j>1-u_{kj}|y_k)
\]
By solving for equality, the expected difference $s_k-s_j$ between latencies can be used to estimate $1-u_{kj}$
\[\mathbb{E}(s_k-s_j|y_k)=\mathbb{E}(1-u_{kj}|y_k)=1-u_{kj}\]
Such expectation minimises the mean squared error
\[
\sum_{\boldsymbol{s}}p(\boldsymbol{s}|y_k)(s_k-s_j-1+u_{kj})^2
\]
Under the assumption $\mathbb{E}(s_k|y_k)\approx 1$, we obtain 
\[\begin{aligned}
 \mathbb{E}(1-s_j|y_k) &= 1-u_{kj} \\
  \mathbb{E}(s_j|y_k) &= u_{kj}
\end{aligned}\]
which has the desirable property that $u_{kj}\approx 1$ when $\mathbb{E}(s_j|y_k)=1$
and $u_{kj} \approx 0$ when $\mathbb{E}(s_j|y_k)=0$. We know that the assumption  $\mathbb{E}(s_k|y_k)\approx 1$ is reasonable, because ECC networks strive to maximise the product $\boldsymbol{x}W$. It also holds that $\mathbb{E}(s_j|y_k)\approx \mathbb{E}(s_j s_k|y_k)$, which implies that $u_{kj}$ is stronger for neurons with similar connectivities $W_k,W_j$. 
