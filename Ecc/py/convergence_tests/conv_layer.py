...
class HardWtaL2:
    ...
    def __init__(self, shape: ecc_py.ConvShape, l=2, w_step=0.0001, r_step=1. / 1024 * 2):
        self.r_step = r_step
...
def from_tag(tag: str, m, n, weight_step=0.0001, inhibition_step=0.001):
    if tag == "hwtaL1":
...
def parse_shape(code: str, width, height) -> [ecc_py.ConvShape]:
    """
    :param code: of the form (k[0-9]+s[0-9]+c[0-9]+_)*c[0-9]+
    :return:
    """
    codes = code.split("_")
    r = re.compile(r"k([0-9]+)s([0-9]+)c([0-9]+)")
    shapes = []
    m = re.fullmatch("c([0-9]+)", codes[0])
    in_channels = int(m.group(1))
    in_shape = (height, width, in_channels)
    for code in codes[1:]:
        m = r.fullmatch(code)
        kernel = int(m.group(1))
        stride = int(m.group(2))
        out_channels = int(m.group(3))
        shape = ecc_py.ConvShape.new_in(in_shape, out_channels, kernel, stride)
        shapes.append(shape)
