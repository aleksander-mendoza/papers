...
pub trait Shape<T> {
    ...
    // fn pos_iter_<'a, 'b>(&'a self, pos: &'b mut Self) -> PosIter<'b, 'a, T, Self> where Self: Sized {
    //     PosIter { pos, shape: self, _p: PhantomData }
    // }
}

impl<T: Debug + Rem<Output=T> + Div<Output=T> + Mul<Output=T> + Add<Output=T> + Copy + Zero + One + Ord + Sub<Output=T>> Shape<T> for [T] {
    type O = Vec<T>;
...
impl<T: Debug + Rem<Output=T> + Div<Output=T> + Mul<Output=T> + Add<Output=T> + Copy + Zero + One + Ord + Sub<Output=T>, const DIM: usize> Shape<T> for [T; DIM] {
    type O = Self;
...
pub fn is_valid_pos<T: Copy + Ord + Zero>(shape: &[T], pos: &[T]) -> bool {
    shape.len() == pos.len() && pos.all_ge_scalar(T::zero()) && shape.all_gt(pos)
}

pub fn last_pos_<T: Copy + Zero + One + Ord + Sub<Output=T>>(shape: &[T], pos: &mut [T]) -> bool {
...
pub fn prev_pos_<T: Copy + Zero + One + Ord + Sub<Output=T>>(shape: &[T], pos: &mut [T]) -> bool {
...
pub fn next_pos_<T: Copy + Ord + Add<Output=T> + Zero + One>(shape: &[T], pos: &mut [T]) -> bool {
    for i in 0..shape.len() {
        if pos[i] < shape[i] {
            pos[i] = pos[i] + T::one();
            return true;
...
pub fn pos_<T: Debug + Rem<Output=T> + Div<Output=T> + Copy + Zero + PartialEq>(shape: &[T], mut index: T, pos: &mut [T]) {
...
pub fn idx<T: Debug + Mul<Output=T> + Add<Output=T> + Copy + Zero + PartialOrd>(shape: &[T], pos: &[T]) -> T {
    let mut idx = T::zero();
...
//
// pub struct PosIter<'a, 'b, T, P: Shape<T>> {
//     pos: &'a mut P,
//     shape: &'b P,
//     _p: PhantomData<T>,
// }
//
// impl<'a, 'b, T, P: Shape<T>> Iterator for PosIter<'a, 'b, T, P> {
//     type Item = &'a P;
//
...
//
//
// pub struct PosIterRev<'a, 'b, T, P: Shape<T>> {
//     pos: &'a mut P,
//     shape: &'b P,
//     _p: PhantomData<T>,
// }
//
//
// impl<'a, 'b, T, P: Shape<T>> Iterator for PosIterRev<'a, 'b, T, P> {
//     type Item = &'a P;
//
//     fn next(&mut self) -> Option<Self::Item> {
