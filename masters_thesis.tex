...
\titlePL{Geometria Inteligencji}
\titleEN{Geometry of Intelligence}
...
\supervisor{prof. Wojciech Pałubicki}
% --- Data złożenia pracy (Miasto, miesiąc rok)
\date{Poznań, Wrzesień 2022}
...
\stdate{Poznań, Wrzesień 30, 2022 r.}
...
\begin{streszczenie}
	Niniejsza praca porusza problemy niewydajności uczenia przez wzmocnienie oraz katastroficznego zapominania. Czerpie inspiracje z neuronauki oraz matematycznych podstaw uczenia głębokiego i procesów decyzyjnych Markowa. Zaproponowana jest matematyczna teoria, która jednoczy obserwacje wymienionych dziedzin i daje nowe spojrzenie na problemy uczenia przez wzmocnienie. Na jej podstawie zaproponowany jest nowy model sieci neuronowych, którego celem jest uczenie rzadkich wektorów binarnych, reprezentacja map kognitywnych oraz formalizacja niektórych obliczeniowych aspektów biologicznych neuronów.
	
	\paragraph{Słowa kluczowe:} hipokamp, uczenie przez wzmocnienie, teoria prawdopodobieństwa, algebra abstrakcyjna, żadkie wektory binarne, uczenie poprzez konkurencję, neuronauka
\end{streszczenie}

\begin{abstract}
	This thesis focuses on investigating the problems of sample inefficiency and catastrophic forgetting in reinforcement learning. It takes inspiration from neuroscience, mathematical foundations of deep learning and Markov decision processes. A new theoretical framework is proposed, which unifies all those fields and provides valuable fundamental insights to the many problems of deep reinforcement learning and beyond. Based on those arguments, a new model of neural networks is proposed, with the aim of learning sparse binary vectors, representing cognitive maps and formalising some of the computational mechanisms of biological neurons.
...
\subsection{Inference of groups as autoregressive problem}
...
\]
In practice $p$ is not given a priori and must approximated by learning $q$
\[
p(s'|C_i,a) \approx q(s'|C_i,a) = \sum_{C_j\in \mathcal{C}} q(s'|C_j)q(C_j|C_i,a)
\]
which is a predictive model (an agent) of the environment $S$ with $\mathcal{C}$ as latent variables (in a hidden layer) acted upon by $a$ (agent actions). It can be easily extended to learning from noisy signals $x$ conditional on state $p(x|s)$.

The problem of finding $q$ and $\mathcal{C}$ can be made easier by introducing biases that exploit two properties of $p$.
If $\mathcal{C}$ were cosets then $C_ia\in \mathcal{C}$ and $p(C_ia|C_i,a)=1$, hence, we know that the true $p$ would simplify to $p(s'|C_i,a)=p(s'|C_ia)$. Moreover, cosets must have equal size. This does not imply that $p(C_i)$ must be uniform but biasing the training towards such uniformity might be beneficial regardless. This bias is introduced in later sections under the name of \textit{entropy maximization algorithm}.
