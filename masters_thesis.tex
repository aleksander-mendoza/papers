...
\subsection{Inductive inference of monoids}
...
\begin{example}
An example of convolutional ECC network with 3 columns (each column is an independent ECC network), kernel size $2$ and stride $1$.
	\begin{figure}[!htbp]
		\centering
...
\end{theorem}
\begin{proof}
Put $V=1-U$ and consider $v_{kj} \in \{0,1\}$. If $v_{kj}=1$ then $u_{kj}=0$ and $s_j+u_{kj}=s_j<s_k$. If $v_{kj}=1$ then $u_{kj}=1$ and is $s_j+u_{kj}<s_k$ always false because $s_k \le  1$. 
\end{proof}
...
\subsection{Learning soft-WTA $U$ weights}
\iffalse
...
\begin{definition}
	\textit{Bursting ECC network} is an incremental ECC network in which the output pattern $\boldsymbol{y}^{(t)}$ is at random time steps $t$ set equal to the pattern in previous step $\boldsymbol{y}^{(t-1)}$. 
\end{definition}
\begin{definition}
	\textit{Slow binary feature analysis} problem is the formulated as minimization of $\frac{d\boldsymbol{y}}{dt}$. As the bits of $\boldsymbol{y}$ are mutually-exclusive  is an incremental ECC network in which the output pattern $\boldsymbol{y}^{(t)}$ is at random time steps $t$ set equal to the pattern in previous step $\boldsymbol{y}^{(t-1)}$. 
\end{definition}
The weights $W$ at $t$ are reinforced between $\boldsymbol{y}^{(t)}$ and $\boldsymbol{x}^{(t)}$

The ECC networks can perform well on simple tasks such as MNIST. Figure \ref{fig:convergence_rate} shows the converge speed and figure \ref{table:ecc_accuracy} shows the accuracy.
\begin{figure}[!htbp]
	\centering
...
\subsection{Causality models of soft- and hard-WTA}
