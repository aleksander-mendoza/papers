...
\label{subsection:inductive_inference_of_monoids}
...
\iffalse
\subsection{On inference of cosets}
Let $S$ be a group of size $n\in \mathbb{N}$. Let $m$ be some number. The task is to find subgroup $G$ of index $[S:G]=m$ (we assume such $G$ exists). To achieve this, we need to find partition $\mathcal{C}=(C_1,C_2,...C_m)$ of $G$ 
\[
...
C_ia=\{sa : s\in C_i\}
\]
Such partition can be found by brute force approach. We seek a more efficient algorithm, which in later sections will be used to derive models of neural networks capable of learning groups embedded in Markov decision processes.
...
\]
which is a predictive model (an agent) of the environment $S$ with $\mathcal{C}$ as latent variables (in a hidden layer) acted upon by $a$ (agent actions). 
If $\mathcal{C}$ are cosets then $C_ia\in \mathcal{C}$ and $p(C_ia|C_i,a)=1$, hence, the model simplifies to $p(s'|C_i,a)=p(s'|C_ia)$.

If $\mathcal{C}$ are cosets and $\omega$ is a permutation on $m$ numbers then $(C_{\omega 1},...,C_{\omega m})$ are costets too. Let $\mathcal{G}_m$ be the set of all subgroups $G<S$ of index $[S:G]=m$. Let $\mathcal{C}_*$ be the set of all  partitions $\mathcal{C}$ that are cosets. There are $m!$ possible permutations $\omega$, therefore, $|\mathcal{C}_*|=|\mathcal{G}_m| m!$. 

The squared probability $p(s'|C_i,a)^2$ can be used to measure ``fitness'' of partition $\mathcal{C}$ \[
\mathbb{E}[p(s'|C_i,a)|C_i,a] = \sum_{s} p(sa|C_i,a)p(s|C_i) = 
\]
...
\subsection{Learning monoids embedded in MDP}
...
\iffalse
\subsection{Learning quotients of monoids embedded in MDP}
...
\subsubsection{Binary vector subsets and their overlap probabilities} 
...
\subsubsection{Formal definition of (first-order) ECC network}
...
\subsubsection{Learning is achieved by maximizing $q(\boldsymbol{x}|y_k)$}
...
\subsubsection{Clustering problem measured with overlap}
...
\subsubsection{Expected overlap and $Q$ matrix} 
...
\subsubsection{Zero-order ECC networks}
...
\iffalse
\subsubsection{Optimal clustering maximizes $p(\boldsymbol{x}|y_k)$} 
...
\subsubsection{Expected value $\mathbb{E}_{p}(\boldsymbol{x}|y_j)$ maximizes $\boldsymbol{x}W_j$ under $\ell_2$ norm} 
...
\subsubsection{One-hot vector maximizes $\boldsymbol{x}W$ under $\ell_1$ norm}
We wish to maximize the expected value of $\boldsymbol{x}W_j$ 
\[
...
\subsubsection{Generalized $a_j$-hot vector maximizes $\boldsymbol{x}W$ under $\ell_1$ norm}
...
\subsubsection{Proof of k-means convergence for first-order networks}
...
\subsubsection{Convergence is not guaranteed for zero-order networks}
...
\subsubsection{Convergence of $Q$ in online-learning}
...
\subsubsection{Higher-order networks}
...
\subsubsection{Probability distribution of $\argmax$}
...
\subsubsection{Soft winner-takes-all mechanism} 
...
\subsubsection{Causality models of soft- and hard-WTA}
...
\subsubsection{Sums $s_j,s_k$ represent similarity of $W_j,W_k$ weight columns}
...
\subsubsection{Real-valued generalization of soft-WTA}
...
\subsubsection{Soft-WTA from SFA perspective}
...
\subsubsection{Learning soft-WTA $U$ weights}
\iffalse
...
\subsubsection{Soft-WTA with $u_{kj}=\mathbb{E}(|s_k-s_j|)$ from high-dimensional k-means perspective}
