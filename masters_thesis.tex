\title{Geometric theory of reinforcement learning}
...
\begin{abstract}
This thesis investigates the problems of sample inefficiency and catastrophic forgetting. It uses an algebraic approach to study group symmetries in reinforcement learning environments. Significant results of this thesis are (1) proof that  finitely generated groups are learnable in the limit, (2) a formal model of winner-takes-all binary neural networks along with proof of convergence and (3) its generalization to multiple winners. The thesis highlights interesting connections to many existing theories of mathematics (geometric deep learning, predictive coding, inductive inference, Bayesian inference, slow feature analysis) and neuroscience (Tolman's cognitive maps, place/ grid/HD cells, spiking networks). The proposed geometric theory of reinforcement learning could be seen as a framework unifying geometric deep learning, Markov decision processes and cognitive maps, potentially offering insights into solving sample inefficiency. Experimental proofs of the concept show empirical validity of the proposed framework, albeit the results remain far from being applicable to ``real world'' scenarios. 
\end{abstract} 
\tableofcontents
...
s = a_1 a_2 a_3 ... a_t
\]
of one or more elements $a_1,a_2,a_3,...a_t$ of $A$. In order to preserve the same notation when $S$ is a monoid or semigroup, let us assume that if $S$ is a group then for every $a\in A$ its inverse $a^{-1}$ is also in $A$ (we won't be using $A^{\pm}$ notation like some authors do). This consequently requires that every presentation $\langle A | \mathcal{R} \rangle$ includes relations $a=a^{-1}$ in $\mathcal{R}$ (or that they can be derived from other relations). In other words, $\langle A | \mathcal{R} \rangle$ is a presentation of monoid $S$ that also happens to be a group. 
...
A question naturally comes to mind, whether finitely presented monoids are learnable too. It seems as though the answer is ``no'' but it remains to be proven. 

\iffalse
\subsubsection{Inductive inference of monoids}

Monoids are more general than groups but they are learnable in the limit as well. The algorithm is analogical. Let $S$ be a monoid presented as $\langle A | \mathcal{R}^* \rangle$ but algorithm $\mathcal{A}$ does not know $\mathcal{R}^*$ a'priori. Let us recall that monoids might not have inverses, thus $\mathcal{A}$ can no longer look for relators. It must find equality relations $w_1=w_2$ where neither of the words $w_1,w_2$ needs to be $e$.

$\mathcal{A}$ proceeds as follows. Define $n \in \mathbb{N}$, $S_{<n}$ and $R_{ss'}^{(i)}$ as before. Define $R_{ss'}^{(<k)}$ as the union $\bigcup_{0 < t< k} R_{ss'}^{(k)}$. It can be seen that $R_{ss'}^{(< k)}$ holds any (observed) word $w$ of length $0<|w|< k$ such that $sw=s'$. We say that a word $w_1$ is inconsistent  with $w_2$ if and only if there is some $s,s'\in S_{<n}$ such that
\[
w_1 \in R_{ss'}^{(<n)} \text{ and } \exists_{s''\ne s'} w_2 \in R_{ss''}^{(<n)}
\]
A pair of words $w_1,w_2$ is consistent if neither is inconsistent with the other.
Relation $w_1=w_2$ could belong to $\mathcal{R}^*$ only if $sw_1=sw_2$ holds for all $s\in S$. If $\mathcal{A}$ observes $sw_1=sw_2$ for some specific $s\in S_{<n}$ then all $\mathcal{A}$ knows is that there exists some $w_3w_1=w_4w_2 \in \mathcal{R}^*$ but the prefixes $w_3,w_4$ remain unknown. We define a relation $w_1'=w_2'$ to be redundant if and only if for all $R_{ss'}^{(<n)}$ containing both $w_1',w_2'$ there exist subwords $w_1'=w_3w_1$, $w_2'=w_4w_2$ such that suffixes $w_1,w_2$ are consistent and the prefixes lead to a common state $s''$
\[
\exists_{s'' \in S_{<n}} w_3,w_4 \in R_{ss''}^{(<n)}
\] 
If such a state $s''$ exists, then $w_1=w_2$ already explains $w_1'=w_2'$ (note this is a recurring theme in inference theory, where picking shortest explanations is crucial for learning in the limit).
Therefore the final result $\mathcal{R}\subset A^*$ must consist of all non-redundant relations $w_1'=w_2'$ such that $w_1,w_2 \in R_{ss'}^{(<n)}$ are consistent. Consistency ensures that if $sw_1'=sw_2'$ but $w_1'=w_2'$ is generally false in $\mathcal{R}^*$ then for some large $n$ a counterexample will be observed that will make $w_1$ and $w_2$ inconsistent. Non-redundancy ensures that once we observe all (finitely many) relations in $\mathcal{R}^*$  and all the derivative (implied) relations of some bounded length then $\phi$ cannot ``fool'' us by presenting some new $sw_1'=sw_2'$ that is generally false (there might be infinitely many such $w_1',w_2'$) but true only in presence of some prefixes $w_3,w_4$ converging in a common state $s''$. This guarantee is weaker then before, because it is possible to find some $\mathcal{R}$ equivalent to the true $\mathcal{R}^*$ and then (briefly) introduce a generally false relation but for a large enough $n$ all derivative relations (of bounded length) will be observed and $\mathcal{A}$ could no longer be ``fooled''.
This concludes $\mathcal{A}$.

The full proof of learning in the limit is mostly analogical as before and the significant differences have already been pointed out. All the quantifiers used in $\mathcal{A}$ were applied to finite domains, ensuring that $\mathcal{A}$ halts in finitely many steps.
It only remains to highlight that coset enumeration can no longer be applied for monoids and a different planning method needs to be devised.
\fi

\subsection{Group manifolds}
For an agent moving in a large room, the state space $S$ might look like a group of two-dimensional translations... until it hits a wall. To rigorously discuss this phenomenon, we need to assume that $S$ is a monoid generated by $A$. Take a subset $E \subset S$ but not necessarily a submonoid. We define that $E$ is isomorphic to a monoid $G$ if there exists a mapping $f:S \rightarrow G$ such that $f(e)a=f(ea)$ for all $e\in E$, $a\in A$. It is a relaxed definition of monoid isomorphism, where $E$ need not be closed under the monoid operation. We should notice that $A$ plays an important role. Depending on the chosen generator, the isomorphism may or may not hold. Recall that $A$ holds both $a$ and $a^{-1}$ if the inverse exists. It is possible that two elements $a_1,a_2 \in A$ are not inverse in general but $a_1a_2=e$ holds within some $E$. If $G$ is a group then let us refer to $E$ as a \textit{group manifold} embedded in $S$ which locally resembles $G$.

$A$ can be used to encode the distance. Fix the smallest possible generator $A$. Let $E$ be isomorphic to $G=\langle A | \mathcal{R} \rangle$. We say that $E$ is isomorphic to $G$ with radius $r$ if it is isomorphic to $\langle B|\mathcal{Q} \rangle$ where each word in $h\in A^{< r}=\bigcup_{0 < t< r} A^t$ is treated as an individual symbol $b_{h}\in B$ and $\mathcal{Q}$ contains all the necessary relations so that $\langle B|\mathcal{Q} \rangle = G$. Equivalently $E$ is isomorphic to $G$ with radius $r$  if and only if there exists $f$ such that $f(s)a^t=f(sa^t)$ for all $0\le t < r$, $s\in E$. 

We can generalize further by assuming that $S$ is also a metric space with metric function $d$. We say that  $E$ is isomorphic to $G$ with radius $r$  if and only if  there exists $f$ such that $f(s)a=f(sa)$ for all $a\in S$, $s\in E$ such that $d(s,sa)<r$.  When $d(s,s')=t$ is a distance defined as the smallest $t=|h|$ such that $sh=s'$, then $d$ induces discrete topology on $S$ that coincides with the previous definitions. 


We define $E$ to be dense in $S$ if it is a subset of $S$ such that for every element of $s\in S$ there exists $e\in E, a\in A$ such that $ea=s$. 
...
A recurring theme in algorithmic learning theory and inductive inference is finding the shortest algorithm that explains (decides) all observations. Occam's razor states that the simplest explanations have the highest likelihood of being correct, which has been formalized by Kolmogorov complexity and algorithmic information theory \cite{Shen2017KolmogorovCA}. Perhaps we could follow these theories and define the problem of general intelligence as the problem of inferring (finding) the simplest group $G$ that has a dense span $E$ in $S$. By ``simplest'' we mean a group $G$ that has the smallest non-trivial quotient group $H$.  The smaller $H$ the more ``regular'' $G$. Question of what to do when $H$ is a quotient of several candidates $G$ remains open.

The biological brain is good at building cognitive maps, which encode many geometric properties of the world. Grid cells fire in patterns that we could interpret as cosets of 2-dimensional translation group. Place cells represent cosets of even finer granularity. Head-direction cells are cosets of rotation group. 

An artificial intelligence algorithm similar to the brain should be capable of decomposing $S$ into a product of subgroups at many different scales of granularity. If $S = Z_1 \times Z_2 \times Z_3 \times ... \times Z_k$ then the size $\vert S\vert = \vert Z_1 \vert  \cdot \vert Z_2 \vert  \cdot ... \cdot \vert Z_k\vert$ would grow exponentially, thus it could be a very efficient representation of $S$. It remains to be investigated how the notion of (semi-) direct product could be generalized to group manifolds. Perhaps using homomorphisms that project an open ball and all smaller balls contained in it onto a single point? For example imagine moving in a labyrinth of rooms and corridors. The physical walls that make up the labyrinth are embedded in euclidean space with translation and rotation symmetries. What if the structure of the corridors follows a regular pattern that looks like some other group? Perhaps if such process of abstraction continued far enough, we could produce agents capable of symbolic reasoning and understanding complex mathematics. Perhaps groups are too restrictive and learning monoids or semigroups would be necessary.
